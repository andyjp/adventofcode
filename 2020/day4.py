...
passports = [p for p in passports if len(expected_fields.intersection(re.findall('[a-z]{3}(?=:)', p))) == 7]
...
field_validation = {
  ...
  'hgt': lambda i: re.match('\d*(cm|in)',i) and (150 <= int(re.findall('\d*',i)[0]) <= 193 if 'cm' in i else 59 <= int(re.findall('\d*',i)[0]) <= 76),
  'hcl': lambda i: re.match('#[0-9a-f]{6}',i),
  'ecl': lambda i: i in ['amb', 'blu', 'brn', 'gry', 'grn', 'hzl', 'oth'],
  'pid': lambda i: re.match('^[0-9]{9}$',i),
  'cid': lambda i: True
}

def is_valid(p):
  fields=[s.split(':') for s in re.split('\s', p)] 
  for [key, value] in fields:
    if not field_validation[key](value):
      return False
  return True

print('Part 2:', len([p for p in passports if is_valid(p)]))
