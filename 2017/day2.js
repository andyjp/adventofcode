...
// Part 1
console.log(rows.reduce((accum, row) => {
    const numbers = row.split(/\t/);
    const parsedNums = numbers.map((n) => parseInt(n));
    return accum += Math.abs(Math.max(...parsedNums) - Math.min(...parsedNums))
}, 0))

// Part 2
const divison = (numbers) => {
    [firstN, ...rest] = numbers;
    const result = rest.reduce((accum, n) => {
        if (accum !== 0) return accum;
        if (n > firstN && n % firstN === 0) return n/firstN;
        if (n < firstN && firstN % n === 0) return firstN/n;
        return accum;
    }, 0);
    return result > 0 || rest.length === 0 ? result : divison(rest);
};

console.log(rows.reduce((accum, row) => {
    const numbers = row.split(/\t/);
    const parsedNums = numbers.map((n) => parseInt(n));
    return accum += divison(parsedNums)
}, 0))