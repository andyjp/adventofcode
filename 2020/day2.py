...
count = 0
for (mn, mx, letter, psswd) in input:
  if mn <= len([l for l in list(psswd) if l == letter]) <= mx:
    count = count + 1
print(len([psswd for (mn, mx, letter, psswd) in input if mn <= len([l for l in list(psswd) if l == letter]) <= mx]))

count = 0
for (pos_1, pos_2, letter, psswd) in input:
  if (psswd[pos_1-1] == letter) ^ (psswd[pos_2-1] == letter):
    count = count + 1
print(count)