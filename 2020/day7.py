input="""dim silver bags contain 3 posh fuchsia bags.
...
for rule in input.split('\n'):
  ...
  for i in inner.split(', '):
    i = i.replace('bags', '').replace('bag', '').strip('. 1234567890')
    if i in bags:
      bags[i].append(outer)
    else:
      bags[i] = [outer]
...
print('Part 1:', len(set(get_it('shiny gold')))-1)
...
def get_em(bag):
  match = re.search('^{} bags contain .*'.format(bag), input, re.MULTILINE)
  inners = re.findall('\d \w* \w* bag', match.group(0))
  count = 0
  for i in inners:
    m = re.match('(\d) (\w* \w*) bag', i).groups()
    count += int(m[0]) + int(m[0]) * get_em(m[1])
  return count

print('Part 2:', get_em('shiny gold'))