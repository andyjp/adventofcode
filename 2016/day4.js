...
for (name of input) {
    ...
    if (name.indexOf("[" + getCheckSum(name) + "]") > -1) {

        // get sector id and add it
        let sectorId = parseInt(name.match(/([0-9]+)/));

        // number of characters to move
        let charMove = sectorId % 26;

        words = name.match(/([a-z]+)(?=-)/g);

        let roomName = name.split("").filter(l => /[a-z-]/.test(l)).map((l) => {
            if (["-"].indexOf(l) > -1) {
                return " ";
            } else {
                // ascii is 97 to 122
                return l.charCodeAt(0) + charMove > 122 ? String.fromCharCode(l.charCodeAt(0) + (charMove -26)) : String.fromCharCode(l.charCodeAt(0) + charMove)
            }
        }).join("");
        ...
        //
        //         // console.log(words[i])
        //         let newLetter = words[i].charCodeAt(j) + charMove > 122 ? String.fromCharCode(words[i].charCodeAt(j) + (charMove - 26)) : String.fromCharCode(words[i].charCodeAt(j) + charMove);
        //
...
// get check sum
// i.e. first 5 most common letters in input
function getCheckSum(input) {

    // initialize a map where the keys will be the letter and the value will be the count
    ...
    for (let i = 0; i < input.length; i++) {
        ...
        // check to see if this is a letter and if we already did this letter
        if (/[a-z]/i.test(letter) && !letters.has(letter)) {

            // get count of letter occurrence in input
            let count = 0;
            ...
            // loop through checksum and compare the count with the count of the letter at that position
            for (let j = 0; j < checksum.length; j++) {

                let compareTo = letters.get(checksum[j]);

                // compare count
                if (count > compareTo) {
                    checksum.splice(j, 0, letter);
                    added = true;
                    break;
                } else if (count == compareTo) {

                    // if count same compare value to sort alphabetically
                    if (input[i] < checksum[j]) {
                        checksum.splice(j, 0, letter);
                        added = true;
                        break;
                    }
            ...
            // if letter didn't get added and we have less than 5 letters add it to end
            if (!added && checksum.length < 5) {
                checksum.push(input[i])
            }

            // delete if we have more than 5
